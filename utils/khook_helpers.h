...
template <typename RETURN, typename... ARGS> class CKHookFunction : public CKHookBase
{
  public:
    using fnCallback = KHook::Return<RETURN> (*)(ARGS...);

    CKHookFunction(const char* pSignature, fnCallback cbPre, fnCallback cbPost) : m_hook(cbPre, cbPost), m_pSignatureName(pSignature)
    {
    ...
    RETURN CallOriginal(ARGS... args) { return m_hook.CallOriginal(args...); }
...
template <typename RETURN, typename... ARGS> auto MakeKHookFunction(const char* pSignature, KHook::Return<RETURN> (*cbPre)(ARGS...), std::nullptr_t cbPost)
{
    return std::make_unique<CKHookFunction<RETURN, ARGS...>>(pSignature, cbPre, nullptr);
}

template <typename RETURN, typename... ARGS> auto MakeKHookFunction(const char* pSignature, std::nullptr_t cbPre, KHook::Return<RETURN> (*cbPost)(ARGS...))
{
...
#define KHOOK_FUNCTION(name, pre, post) static auto hook##name = MakeKHookFunction(#name, pre, post)

template <typename CLASS, typename RETURN, typename... ARGS> class CKHookMember : public CKHookBase
{
  public:
    using fnCallback = KHook::Return<RETURN> (*)(CLASS*, ARGS...);

    CKHookMember(const char* pSignature, fnCallback cbPre, fnCallback cbPost) : m_hook(cbPre, cbPost), m_pSignatureName(pSignature)
    {
        GetKHookList().push_back(this);
    }
...
#define KHOOK_MEMBER(name, pre, post) static auto hook##name = MakeKHookMember(#name, pre, post)
